...
2. 워크스페이스 선택
3. 아래 manifest를 YAML 탭에 붙여넣기 → **Next** → **Create**

<details>
<summary>📋 Manifest 복사하기</summary>
...
| `SECRET_ACCESS`        | `$AWS_SECRET`, `$SLACK_WEBHOOK`, `process.env.TOKEN` | 0.8 | 승인 요청 |
| `SUSPICIOUS_DOMAIN`    | `.xyz`, `.tk`, `.ml`, `.pw`, `.top`              | 0.8     | 승인 요청 |
| `DATA_EXFILTRATION`    | `curl -X POST`, `nc -e`, reverse shell           | 0.7     | 승인 요청 |
| `ENV_DUMP`             | `printenv`, `env \| grep`, `export -p`, `/proc/*/environ` | 0.6 | 승인 요청 |
| `SENSITIVE_FILE`       | `.env`, `.ssh/id_rsa`, `.aws/credentials`, `/proc/*/environ` | 0.6 | 승인 요청 |
